...
if __name__ == '__main__':
    ...
    # update_daily_data(TOKEN, all_instruments, years=30)
    ...
    for figi in shares["figi"].to_list():
        ...
        if len(df) > 0:
            ...
            df["first_day"] = np.where(df["month_open"] > 0, 1, 0)
            df["month_open"] = df["month_open"].fillna(method="ffill")
            monthly_std = df["close"].resample("M").mean().rolling(14).std().resample("D").mean().bfill().shift(
                1).rename("monthly_std")
            df = pd.concat([df, monthly_std], axis=1)
            df["monthly_std"] = df["monthly_std"].bfill().ffill()
            df["m_std_h"] = df["month_open"] + df["monthly_std"]
            df["m_std_l"] = df["month_open"] - df["monthly_std"]
